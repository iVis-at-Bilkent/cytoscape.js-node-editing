(function () {
    ...
    var register = function (cytoscape, Konva) {
        ...
        cytoscape('core', 'nodeEditing', function (opts) {
            ...
            if (opts === 'get') {
                return scratch.getScratch(cy, 'api');
            }
            ...
            var sizeCanvas = function () {
                canvasElement.setAttribute('height', cy.height());
                canvasElement.setAttribute('width', cy.width());
                ...
                setTimeout(function () {
                    canvasElement.style.top = -(canvasElement.offsetTop - container.offsetTop);
                    canvasElement.style.left = -(canvasElement.offsetLeft - container.offsetLeft);

                    canvas.getStage().setWidth(cy.width());
                    canvas.getStage().setHeight(cy.height());
                }, 0);
            };

            sizeCanvas();

            window.onresize = debounce(sizeCanvas, 250);
            ...
            var ResizeControls = function (node) {
                ...
                var grappleLocations = ['topleft', 'topcenter', 'topright', 'centerright', 'bottomright', 'bottomcenter', 'bottomleft', 'centerleft'];
                this.grapples = [];
                ...
                if (options.resizeToContentCueEnabled(node) && !options.isNoResizeMode(node) && (!options.autoRemoveResizeToContentCue || (options.autoRemoveResizeToContentCue && !isResizedToContent(node)))) this.resizeCue = new ResizeCue(node, this);
            ...
            ResizeControls.prototype.update = function () {
                ...
                var rcEnabled = options.resizeToContentCueEnabled(node) && !options.isNoResizeMode(node) && (!options.autoRemoveResizeToContentCue || (options.autoRemoveResizeToContentCue && !isResizedToContent(node)));

                if (this.resizeCue && rcEnabled) this.resizeCue.update();
                else if (this.resizeCue && !rcEnabled) {
                    this.resizeCue.unbindEvents();
                    this.resizeCue.shape.destroy();
                    delete this.resizeCue;
                } else if (!this.resizeCue && rcEnabled) this.resizeCue = new ResizeCue(node, this);
            ...
            Grapple.prototype.bindInactiveEvents = function () {
                ...
                var eMouseLeave = function (event) {
                    if (event.target.getStage() != undefined) event.target.getStage().container().style.cursor = options.cursors.default;
                };
            ...
            Grapple.prototype.bindActiveEvents = function () {
                ...
                var eMouseUp = function (event) {
                    cy.style().selector('core').style('active-bg-opacity', tmpActiveBgOpacity).update();
                    self.shape.getStage().container().style.cursor = options.cursors.default;
                    cy.boxSelectionEnabled(true);
                    cy.panningEnabled(true);
                    setTimeout(function () {
                        // for some reason, making node unselectable before doesn't work
                ...
                var eMouseMove = function (event) {
                    ...
                    cy.batch(function () {
                        ...
                        if ((isAspectedMode && location.indexOf('center') >= 0) || options.isNoResizeMode(node)) return;
                        ...
                        if (isAspectedMode) {
                            ...
                            if (xWidth > xHeight) xHeight = xWidth * aspectRatio * (isCrossCorners ? -1 : 1);
                            else xWidth = (xHeight / aspectRatio) * (isCrossCorners ? -1 : 1);
                        }
                        ...
                        if (node.isParent()) {
                            ...
                            if (totalExtraWidth > 0) {
                                extraLeft = (totalExtraWidth * parseFloat(options.getCompoundMinWidthBiasLeft(node))) / (parseFloat(options.getCompoundMinWidthBiasLeft(node)) + parseFloat(options.getCompoundMinWidthBiasRight(node)));
                                extraRight = totalExtraWidth - extraLeft;
                            }

                            if (totalExtraHeight > 0) {
                                extraTop = (totalExtraHeight * parseFloat(options.getCompoundMinHeightBiasTop(node))) / (parseFloat(options.getCompoundMinHeightBiasTop(node)) + parseFloat(options.getCompoundMinHeightBiasBottom(node)));
                        ...
                        if (location.startsWith('top')) {
                            ...
                            if (getHeightFcn(node) - xHeight > options.minHeight(node) && (!node.isParent() || extraTop - xHeight >= 0)) {
                                newY = nodePos.y + xHeight / 2;
                                isYresized = true;
                                setHeightFcn(node, getHeightFcn(node) - xHeight);
                            } else if (isAspectedMode) return;
                        } else if (location.startsWith('bottom')) {
                            // Note that xHeight is supposed to be positive
                            // If the node is simple min height should not be exceed, else if it is compound
                            // then extraBottom should not be negative
                            if (getHeightFcn(node) + xHeight > options.minHeight(node) && (!node.isParent() || extraBottom + xHeight >= 0)) {
                                newY = nodePos.y + xHeight / 2;
                                isYresized = true;
                                setHeightFcn(node, getHeightFcn(node) + xHeight);
                            } else if (isAspectedMode) return;
                        ...
                        if (location.endsWith('left') && getWidthFcn(node) - xWidth > options.minWidth(node) && (!node.isParent() || extraLeft - xWidth >= 0)) {
                        ...
                        } else if (location.endsWith('right') && getWidthFcn(node) + xWidth > options.minWidth(node) && (!node.isParent() || extraRight + xWidth >= 0)) {
            ...
            ResizeCue.prototype.bindEvents = function () {
                ...
                var onClick = function (event) {
                    ...
                    } else if (cy.undoRedo && options.undoable) cy.trigger('nodeediting.resizetocontent', [self]);
                    else {
                        var params = {
                            self: self,
                            firstTime: true,
            ...
            var isResizedToContent = function (node) {
                ...
                if (minWidth !== 0) {
                    if (typeof options.isFixedAspectRatioResizeMode === 'function' && options.isFixedAspectRatioResizeMode(node)) {
            ...
            function arrowKeyDownListener(e) {
                var shouldMove = typeof options.moveSelectedNodesOnKeyEvents === 'function' ? options.moveSelectedNodesOnKeyEvents() : options.moveSelectedNodesOnKeyEvents;
    ...
    if (typeof cytoscape !== 'undefined' && typeof Konva !== 'undefined') {
