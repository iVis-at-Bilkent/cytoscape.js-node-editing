  ...
  var register = function (cytoscape, Konva) {
    ...
    cytoscape('core', 'nodeEditing', function (opts) {
      ...
      var ResizeControls = function (node) {
        ...
        for (var i = 0; i < grappleLocations.length; i++) {
          ...
          if (
            ...
            (options.isFixedAspectRatioResizeMode(node) && location.indexOf('center') >= 0)
          ) {
            isActive = false;
          }
        ...
        if (
          options.resizeToContentCueEnabled(node) &&
          !options.isNoResizeMode(node) &&
          (!options.autoRemoveResizeToContentCue || (options.autoRemoveResizeToContentCue && !isResizedToContent(node)))
        )
          ...
          cy.batch(function () {
            ...
            if (node.isParent()) {
              ...
              if (totalExtraWidth > 0) {
                extraLeft =
                  (totalExtraWidth * parseFloat(options.getCompoundMinWidthBiasLeft(node))) /
                  (parseFloat(options.getCompoundMinWidthBiasLeft(node)) +
                    parseFloat(options.getCompoundMinWidthBiasRight(node)));
                extraRight = totalExtraWidth - extraLeft;
              }

              if (totalExtraHeight > 0) {
                extraTop =
                  (totalExtraHeight * parseFloat(options.getCompoundMinHeightBiasTop(node))) /
                  (parseFloat(options.getCompoundMinHeightBiasTop(node)) +
                    parseFloat(options.getCompoundMinHeightBiasBottom(node)));
            ...
            if (location.startsWith('top')) {
              ...
              if (
                getHeightFcn(node) - xHeight > options.minHeight(node) &&
            ...
            if (
              ...
              getWidthFcn(node) - xWidth > options.minWidth(node) &&
      ...
      var isResizedToContent = function (node) {
        ...
        if (minWidth !== 0) {
          if (
            typeof options.isFixedAspectRatioResizeMode === 'function' &&
        ...
        return (
          width === minWidth * resizeToContentSizeMultiplier && height === minHeight * resizeToContentSizeMultiplier
        );
      };
      ...
      function arrowKeyDownListener(e) {
        var shouldMove =
          typeof options.moveSelectedNodesOnKeyEvents === 'function'
            ? options.moveSelectedNodesOnKeyEvents()
            : options.moveSelectedNodesOnKeyEvents;
